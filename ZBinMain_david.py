	...
	try:
		while True:
			...
			if temp_weight>=-10 and temp_weight<0: #rounding negative numbers close to zero to zero
				...
				weight = "NULL" #Note to Owen: did you want this to be 0.0 or null?
			...
			while GPIO.input(ECHO)==0 and '''removes looping issue''' ping <= MAX_ULTRASONIC_PING:
			...
			if ping == MAX_ULTRASONIC_PING || timed_out:
				timed_out = True
...
def update_tippers(WEIGHT_SENSOR_ID, WEIGHT_TYPE, ULTRASONIC_SENSOR_ID, ULTRASONIC_TYPE, HEADERS, BININFO):
	...
	for row in cursor:
		...
		try:
			...
			if distance != "NULL" && not timed_out:
